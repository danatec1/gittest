...
def crawl(keyword: str, max_pages: int, headless: bool, logger) -> list[dict]:
    driver = create_driver(headless=headless)
    try:
        logger.info("start selenium crawl: keyword=%s max_pages=%s", keyword, max_pages)
        ...
        page_rows = extract_items_from_text(body_text, keyword=keyword, page_no=1)
        rows.extend(page_rows)
        logger.info("page=1 rows=%s", len(page_rows))

        for page in range(2, max_pages + 1):
            if not go_to_next_page(driver, page):
                logger.info("stop at page=%s: no next link", page)
                break
            body_text = driver.find_element(By.TAG_NAME, "body").text
            page_rows = extract_items_from_text(body_text, keyword=keyword, page_no=page)
            if not page_rows:
                logger.info("stop at page=%s: no rows", page)
                break
            rows.extend(page_rows)
            logger.info("page=%s rows=%s", page, len(page_rows))

        rows = deduplicate_rows(rows)
        rows = deduplicate_title_type(rows)
        logger.info("final deduped rows=%s", len(rows))
        return rows
    finally:
        driver.quit()
...
def main() -> None:
    settings = Settings()
    logger = setup_logger("crawl_selenium")
    ...
    rows = crawl(keyword=args.keyword, max_pages=args.max_pages, headless=headless, logger=logger)
    df = to_dataframe(rows)

    logger.info("dataframe rows=%s", len(df))
    print(df.head(20))
    print(f"rows={len(df)}")
